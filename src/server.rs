...
use std::{collections::BTreeMap, collections::VecDeque, error::Error, pin::Pin};
...
use crate::config::{AccountKind, EffectiveAccount, EffectiveConfig};
...
pub struct AppState {
    ...
    account_routing: Arc<ArcSwap<Vec<EffectiveAccount>>>,
    upstream_clients: Arc<BTreeMap<String, reqwest::Client>>,
...
impl AppState {
    ...
    fn routing_accounts(&self) -> Arc<Vec<EffectiveAccount>> {
        self.account_routing.load_full()
    }

    fn upstream_client_for_url(
        &self,
        upstream_url: &Url,
...
mod tests {
    ...
    async fn fake_delayed_websocket_upstream(
    ...
    async fn fake_closing_websocket_upstream(
        captured_messages: Arc<Mutex<Vec<String>>>,
    ) -> SocketAddr {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
    ...
    async fn should_timeout_waiting_for_upstream_response_headers() {
        let captured = Arc::new(Mutex::new(Vec::new()));
        let upstream =
            fake_delayed_upstream(Duration::from_millis(50), Arc::clone(&captured)).await;
    ...
    async fn should_record_cached_tokens_from_responses_usage_without_rewriting_body() {
        let captured = Arc::new(Mutex::new(Vec::new()));
        let upstream_body =
            r#"{"id":"resp_ok","usage":{"input_tokens_details":{"cached_tokens":17}}}"#;
...
"#,
    ...
    async fn should_reject_second_websocket_create_while_first_is_in_flight() {
        ...
        assert!(metrics.contains(
            r#"tokenproxy_ws_events_total{event_type="upstream_text",success="true"} 1"#
        ));
    }
